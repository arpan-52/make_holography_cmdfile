...
def Ellipse(xxx_todo_changeme, xxx_todo_changeme1, alpha, nbin=50, **kwargs):
    (cenx,ceny) = xxx_todo_changeme
    (radx, rady) = xxx_todo_changeme1
    import numpy as N
    ...
    return pl.Polygon(list(zip(xs, ys)), closed=True, alpha=alpha, **kwargs)
...
def poshfit_1d(x, y, func, sig=None, mask=None, do_err=False, p0 = None):
    ...
    # mask
    if mask == None: mask = N.zeros(x.shape)
...
def wc(fname):
    ...
    if not os.path.isfile(fname):
      ...
      nn = int(subprocess.getoutput('wc -l '+fname).split()[0])
...
def imshow(im, vmin=None, vmax=None, extent=None):
    import pylab as pl
    import numpy as N
