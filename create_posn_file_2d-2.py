...
def antennaframe(freq=330.e6):
  ...
  time1 = time0+total_time_hr + (grid1d+1)*tot_time_per*ncal/3600.0   # +1 because of initial cal scan
  ...
  times = []
  ctr=1
  for i in range(len(timeflag)):
    if (timeflag[i]==1) or ((i!=len(timeflag)-1) and (timeflag[i]+timeflag[i+1]==0)):   # if grid or beg of cal scan
      times.append(times_all[i])
    if (timeflag[i]==1):
      scannames.append('PM0'+'0'*(3-len(str(ctr)))+str(ctr))
      ctr+=1 
    else:
      if ((i!=len(timeflag)-1) and (timeflag[i]+timeflag[i+1]==0)):  # beg of cal scan 
        scannames.append(source)
  times = N.asarray(times)
...
#k both are wrong u need sph geom dist  do that and u will be fine
  ...
  fn.write("subar 4\n")
  fn.write("dellist 2\n")
  fn.write("addlist '/odisk/gtac/cmd/holo/src_3c147.list'\n\n")
  ...
  for i in range(len(times)):
    ...
    # *** Div 30Apr2019 ***
    fn.write("trkantof(" + str(int(drot_az[i]/N.cos((rot_alt[i])/180.*math.pi)*3600))+'", '+str(int(drot_alt[i]*3600))+'")\n')
